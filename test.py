...
top_frame = Frame(root, bg='Brown', width=50, height=50, pady=3)
center = Frame(root, bg='Brown', width=50, height=40, padx=3, pady=3)
btm_frame = Frame(root, bg='Brown', width=50, height=50, pady=3)
...
ctr_left = Label(center,bg='grey', width=50, height=190)
ctr_mid = Label(center,bg='grey',width=640, height=480, padx=1, pady=1)
...
for index in range(9):
    if len(PHOTOS[index]) > 0:
        for k,photo in enumerate(PHOTOS[index]):
            btn= Button(scrollable_body, highlightbackground='grey', text=IMAGES[index][k].rsplit('/',1)[1].replace('.png','')[:-1],bg='grey', image=photo, command = lambda index=index, k=k: put_sprite(index,k), compound=LEFT, width='500', height='400')
